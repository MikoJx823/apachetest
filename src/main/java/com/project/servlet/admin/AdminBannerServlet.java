...
public class AdminBannerServlet extends HttpServlet {
	...
	private void add(HttpServletRequest request, HttpServletResponse response, SmartUpload su){
		...
		try{
			...
			su.setAllowedFilesList("jpg,JPG,jpeg,JPEG,PNG,png,GIF,gif");
			
			AdminInfoBean loginUser = (AdminInfoBean)request.getSession().getAttribute(SessionName.loginAdmin);
			
			String name = StringUtil.filter(su.getRequest().getParameter("name"));
			String link = StringUtil.filter(su.getRequest().getParameter("link"));
			int position = StringUtil.strToInt(su.getRequest().getParameter("position"));
			int seq = StringUtil.strToInt(su.getRequest().getParameter("seq"));
			...
			for (int i = 0; i < su.getFiles().getCount(); i++){
				...
				if (!(fileExt.toLowerCase().endsWith("jpg") || fileExt.toLowerCase().endsWith("png") || 
					  fileExt.toLowerCase().endsWith("jpeg") || fileExt.toLowerCase().endsWith("gif")) ){
					errorMsg += "Image format is not correct. <br>";
					break;
				}
				
				if ("image".equals(fieldName)){
					bean.setImage("y");
				}else if("imageapp".equals(fieldName)) {
					bean.setAppimage("y");
				}
			}
			...
			if("".equals(errorMsg)) {
				...
				for (int i = 0; i < su.getFiles().getCount(); i++){
					...
					fileName += "_banner." + fileExt;
					file.saveAs(StringUtil.getBasePath() + "images/banner/" + fileName);
				}
	
				//STORE BEAN 
				BannerInfoBean resultProd = BannerService.getInstance().insert(bean);
				if(resultProd!=null){
					errorMsg = "Add Banner success.";
					resultUrl = "bannerView.jsp?id=" + resultProd.getId();							
				}else{
	...
	private void update(HttpServletRequest request, HttpServletResponse response, SmartUpload su) throws IOException {
		...
		try{
			...
			if("".equals(errorMsg)){
				...
				for (int i = 0; i < su.getFiles().getCount(); i++){
					...
					fileName += "_banner." + fileExt;
					//file.saveAs(PropertiesUtil.getProperty("basePath") + PropertiesUtil.getProperty("banner.path") + fileName);
					file.saveAs(StringUtil.getBasePath() + "images/banner/" + fileName);
				}
	...
	private String checkInfo(BannerInfoBean bean){
		...
		/*if("".equals(StringUtil.filter(bean.getLink()))){
			errorMsg += "Link is required. <br>";
		}*/
	...
	private void search(HttpServletRequest request, HttpServletResponse response){
		...
		{	
			...
			String platform = StringUtil.filter(request.getParameter("platform"));
			...
			if ("menu".equals(from)){
				...
				platform = StringUtil.filter((String)request.getSession().getAttribute("platform"));
			}
			...
			if (!"".equals(platform)){
				sqlWhere += " and platform ='" + platform + "'" ;
			}
