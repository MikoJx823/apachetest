								...
								%>
						            <tr class="item">
						                ...
						                <td class="product-thumbnail">
						                    <a href="<%=link%>">
						                        <img src="<%=basePath %>images/products/<%=StringUtil.filter(orderItem.getProductimage()) %>" class="img-responsive" alt="">
						                    </a>
						                </td>
						                ...
						                <td class="product-quantity">
						                    <div class="quantity">
						                        <input type="button" value="+" class="plus" onClick="javascript:changeQty('<%=key %>',<%=orderItem.getQuantity() + 1%>)">
						                        <input type="number" step="1" max="5" min="1" name="qty<%=key %>" id="qty<%=key %>" value="<%=orderItem.getQuantity() %>" title="Qty" class="qty" size="4" disabled>
						                        <input type="button" value="-" class="minus" onClick="javascript:changeQty('<%=key %>',<%=orderItem.getQuantity() - 1%>)">
						                    </div>
						                </td>
...
function changeQty(cartId, newqty) {
    ...
    if(!reg.test(newqty)){ 
		...
		if(newqty < 1){
			...
			$.ajax({
				dataType: 'json',
	            url: '<%=basePath%>cartitemqty?actionType=changeQty&cartId='+cartId+'&qty'+cartId+'='+newqty,
	            type: 'POST',
	            async:false,
